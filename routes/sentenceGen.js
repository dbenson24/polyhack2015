...
function createStory(location)
{
        var string = "";
        var locArray = new Array[6];
        for (var i = 0; i<locArray.length; i++){
                locArray[i]=location.data[i].type;
        }
        var location = "Medford, MA"
       ...
       // for (var i = 0; i<prefixes.length; i++){
                randint = Math.random()*verbs.length;
                verb = verbs[randint];
                randint = Math.random()*adverbs.length;
                adverb = adverbs[randint];
                joke = js.joke_generator(type);
                string +=  "Skeleton" +" "+ verb +" "+ adverb + " to " + attraction + ", and " +joke".";
        //}
